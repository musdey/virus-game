...
public class HighscoreState extends State {
    ...
    private void setupPreferences(){
        ...
        if(!prefs.contains(Config.PREFS_HIGHSCORE)){
            prefs.putInteger(Config.PREFS_HIGHSCORE,0);
            prefs.flush();
        };
        highScore = prefs.getInteger(Config.PREFS_HIGHSCORE);
    }
    ...
    private void setupMenu(){
        ...
        shareBtn.addListener(new ClickListener(){
            ...
            public void clicked(InputEvent event, float x, float y) {
                CoronaGame.shareController.shareWithFriends(
                        "Your friend just reached the highscore of "+highScore+". Can you beat that? Got to http://musdey.cloud and download the game."
                );
            }
        });
